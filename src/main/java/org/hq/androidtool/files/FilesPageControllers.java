...
import org.hq.androidtool.alert.CustomAlerts;
...
public class FilesPageControllers {
    ...
    public FilesPageControllers(Device device, GuiMainController guiMainController) {
        this.device = device;
...
//        System.out.println("Droping Butons from: " + idFromDrop + " to " + pnlHistory.getChildren().size());
        ...
        if (fileDevice != null && fileDevice.getFileType().equals(FilesType.FOLDER)) {
            ...
            if (nameFolder != null)
                if (filesController.mkdir(nameFolder, pathFolder))
                    showMessage(AlertType.SUCCESS, nameFolder, pathFolder);
                else
                    showMessage(AlertType.ERROR, nameFolder, pathFolder);
        } else {
            if (nameFolder != null)
                if (filesController.mkdir(nameFolder, actualPath))
                    showMessage(AlertType.SUCCESS, nameFolder, actualPath);
                else
                    showMessage(AlertType.ERROR, nameFolder, actualPath);
        }
    }
    ...
    private void showMessage(AlertType alertType, String... data) {
        if (alertType.equals(AlertType.SUCCESS))
            alertController.showNotification(
                    Messages.CREATE_FOLDER_SUCCESS.getTitle(),
                    Messages.CREATE_FOLDER_SUCCESS.getMessage(data),
                    10,
                    alertType
            );
        if (alertType.equals(AlertType.ERROR))
            alertController.showNotification(
                    Messages.CREATE_FOLDER_ERROR.getTitle(),
                    Messages.CREATE_FOLDER_ERROR.getMessage(data),
                    10,
                    alertType
            );
    }
